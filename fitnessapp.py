...
def create_database():
    ...
    # Create routine excersites junction table
    cursor.execute('''
...
def view_exercises_by_category():
    ...
    try:
        ...
        cursor.execute("SELECT name exercise_categories WHERE id=?", (category_id,))
        ...
        if exercises:
            print(f"\nExercise in category '{category[0]}':")
            for id, name, description in exercises:
                ...
                if description:
                    ...
                    print(f"No exercises founf in category '{category[0]}'.")

                    # Option to add exercise
...
def add_exercise(category_id=None):
    ...
    if category_id is None:
        ...
        try:
            category_id = int(input("\nEnter category ID for the new exercises: "))
    ...
    exercise_decription = input("Enter exercise description (optional): ")

    cursor.execute(
        "INSERT INTO exercises (name, category_id, description) VALUES (?, ?, ?)",
        (exercise_name, category_id, exercise_decription)
    )
...
def delete_exercise_category():
    ...
    try:
        ...
        if exercise_count > 0:
            ...
            conn.commit()
            print(f"Category '{category[0]}' and all it's exercises have been deleted.")
